...
- [interface](#interface)
  - [🌵满足 interface 接口不需要显示定义](#🌵满足interface接口不需要显示定义)
  - [🌵显式的验证类型实例是否满足指定接口](#🌵显式的验证类型实例是否满足指定接口)
  - [🚩内嵌 interface 形式](#🚩内嵌interface形式)
  - [🚩interface 变量的内部表示](#🚩interface变量的内部表示)
  - [🚩函数直接返回 interface](#🚩函数直接返回interface)
  - [🚩使用空接口作为函数的参数类型](#🚩使用空接口作为函数的参数类型)
...
为了阐明代码中的内嵌`interface`而产生的运行时空指针的问题，我们需要讨论两个问题：
...
关键在于第二种，`struct`内嵌`interface`，这看起来就很奇怪，这有什么道理？

实际上，`struct`内嵌`interface`和`interface`内嵌`interface`没有任何不同。从本质上来说，均是达到同一目的：**把属于里层的所有方法提升(`promoted`)到外层**。
...
`ps`：如果想要探索更多关于内嵌`interface`的内容，比如为什么`Go`要支持这种内嵌行为，可以阅读参考中的博客。
